...
# HTML Content:
# - User Column: Display Name (Bold) + Username (Small)
# - Cargo Column: ad_title
# - Department Column: user_department
                ...
                <table class="table table-dark table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <!-- USER COLUMN: Name Bold + Username Small -->
                                <div class="d-flex flex-column">
                                    <span class="fw-bold fs-6">{{ event.ad_display_name|default:event.username }}</span>
                                    <span class="small text-muted" style="font-size: 0.85em;">{{ event.username }}</span>
                                </div>
                            </td>
                            <td>
                                <!-- CARGO COLUMN -->
                                {% if event.ad_title %}
                                    <span>{{ event.ad_title }}</span>
                                {% else %}
...
container_id = "projeto-teste-web-1"
container_path = "/app/security_events/templates/security_events/webfilter_v2.html"

b64_content = base64.b64encode(html_content.encode('utf-8')).decode('utf-8')
print(f"Uploading {len(html_content)} bytes to container...")

python_command = f"import base64; open('{container_path}', 'wb').write(base64.b64decode('{b64_content}'))"
cmd = ["docker", "exec", "-i", container_id, "python", "-c", python_command]
